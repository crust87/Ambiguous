...
## 정리를 하자면
개발자가 보기에는 정말 이상한 설명이지만 예를들어 설명하자니 이렇게 된거다. 단말기의 어플리케이션이 유저가 어떤 서비스를 사용하기위한 궁극적인 기능을 제공하지 못하고 더 큰 시스템을 이용하여 서비스를 제공하는 케이스가 있을 것이고, 이때 단말기 상의 어플리케이션이 클라이언트고, 더 큰 시스템이 바로 서버가 될 것이다. 사실 서버-클라이언트 구조는 단순한게 생각하면 서버는 "서비스 제공 소프트웨어"가 될 것이고 클라이언트는 "서비스 이용 소프트웨어라"고 설명될 수도 있다. 마지막에 데이터베이스 이야기를 한 것은 서버-클라이언트가 단순히 사용자 손에 있는 단말기 속 소프트웨어와 원격의 장비라는 의미 뿐 아니라 소프트웨어들간의 관계를 나타내고 있다 이야기 하고 싶었기 때문이다.
